def agendamientoHornos(solicitudesHorneado):
    ...
    duracionItenerario=solicitudesHorneado[0]['tiempo']
    numeroHornos=1
    tiempo=0
    for i in range(len(solicitudesHorneado)):
        if i<len(solicitudesHorneado)-1:
            if solicitudesHorneado[i]['tf'] <= solicitudesHorneado[i+1]['t0'] and duracionItenerario+solicitudesHorneado[i+1]['tiempo'] <= 240:
                duracionItenerario+=solicitudesHorneado[i+1]['tiempo']
            else:
                numeroHornos+=1
                tiempo+=duracionItenerario
                duracionItenerario=solicitudesHorneado[i+1]['tiempo']
        elif i==len(solicitudesHorneado)-1:
            tiempo+=duracionItenerario
            if solicitudesHorneado[i-1]['tf'] <= solicitudesHorneado[i]['t0'] and duracionItenerario+solicitudesHorneado[i]['tiempo'] <= 240:
                duracionItenerario+=solicitudesHorneado[i]['tiempo']
            else:
                numeroHornos+=1
                duracionItenerario=solicitudesHorneado[i]['tiempo']
                tiempo+=duracionItenerario
    tiempo_promedio=round(tiempo/numeroHornos,2)
